...
public class GamePane extends GameGrid
{
  ...
  // Detect whether two players are at the same position.
  public boolean detectCollision() {
    Puppet currentPuppet = getPuppet();
    Puppet previousPuppet = puppets.get((currentPuppetIndex - 1 + numberOfPlayers) % numberOfPlayers);

    if (currentPuppet.getCellIndex() == previousPuppet.getCellIndex())
      return true;
  ...
  public Puppet getNextPuppet(){
    if(currentPuppetIndex+1 < numberOfPlayers){
      return puppets.get(currentPuppetIndex+1);
    }
